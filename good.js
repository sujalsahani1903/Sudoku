...
function handleOptionClick(event) {
    ...
    const previousValue = game.grid[row][col];

    game.undoStack.push({ row, col, value: previousValue });
    game.redoStack = [];
    updateUndoRedoButtons();
...
function undo() {
    ...
    const lastMove = game.undoStack.pop();
    const { row, col, value } = lastMove;
    const currentValue = game.grid[row][col];
    game.redoStack.push({ row, col, value: currentValue });
...
function redo() {
    ...
    const lastMove = game.redoStack.pop();
    const { row, col, value } = lastMove;
    const currentValue = game.grid[row][col];
    game.undoStack.push({ row, col, value: currentValue });
    game.grid[row][col] = value;
    const tile = document.getElementById(`${row}-${col}`);
    tile.innerText = (value === 0) ? "" : value;
    updateUndoRedoButtons();
...
function updateUndoRedoButtons() {
    ...
    redoButton.disabled = game.redoStack.length === 0;
}
